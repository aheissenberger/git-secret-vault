...
- FR-002 | InProgress | Use a dual-profile vault architecture: authoritative on-disk storage using a content-addressed blob store with a JSONL event log, plus a ZIP interchange format for export/import only.
...
- FR-030 | InProgress | Store each encrypted secret as a content-addressed blob at `blobs/<sha256-of-plaintext>.enc` using XChaCha20-Poly1305 AEAD with a 24-byte random nonce prepended to the ciphertext.
- FR-031 | InProgress | Maintain an append-only JSONL event log at `index/events.jsonl` recording all vault mutations with required fields: `timestamp`, `op`, `entry_id`, `content_hash`, `key_id`.
- FR-032 | InProgress | Maintain a canonical snapshot at `index/snapshot.json` sorted by `entry_id`, regenerated by replaying `index/events.jsonl`, for fast vault state recovery without full log replay on every operation.
- FR-033 | InProgress | Store vault-level metadata in `vault.meta.json` with fields: `version`, `kdf_params` (Argon2id parameters), `crypto_suite` (identifier string), and `key_ids` (array). The file must not contain plaintext secret paths or filenames.
- FR-034 | InProgress | Provide `vault zip-export` and `vault zip-import` commands that produce and consume AES-256 AE-2 encrypted ZIP files for interchange. ZIP is not authoritative vault storage.
...
- NFR-003 | InProgress | ZIP is the interchange export format only; authoritative vault storage is the dual-profile blob/index structure. Standard unzip tools remain usable via explicit export.
...
- SEC-003 | InProgress | Encrypt each blob with XChaCha20-Poly1305 AEAD using a per-blob random nonce; derive vault keys using Argon2id KDF with parameters stored in `vault.meta.json`.
- SEC-004 | InProgress | Prepend a 24-byte random XChaCha20 nonce to each blob ciphertext; nonces are never reused or derived deterministically.
